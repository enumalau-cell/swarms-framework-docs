...
Use a wait limit when your service needs to continue while an agent is still
working. `future.result(timeout=30)` alone does not bound a surrounding
`with ThreadPoolExecutor(...)` block: leaving that block waits for the worker.

Manage shutdown explicitly in this example. `wait` reports completion without
catching exceptions raised by the agent. Read a completed future with
`future.result()` so failures, including the agent's own `TimeoutError`, still
reach the caller.
...
try:
    ...
    completed, _ = wait([future], timeout=30)
    if completed:
        result = future.result()
    else:
        logger.warning("30-second wait ended; the agent may still be running")
finally:
    executor.shutdown(wait=False, cancel_futures=True)
```

`shutdown(wait=False)` lets the calling code continue. It does not stop an
already running thread, and Python still waits for that thread before exiting.
`cancel_futures=True` only cancels work that has not started.

Keep the future if you need to collect a late result or exception. Do not launch
an automatic retry or reuse the same agent while this run may still be active.
For a deadline that actually ends work, configure timeouts in the network clients
and tools used by the agent, or use a worker process whose lifecycle you control.
